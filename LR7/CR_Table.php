<?php
    ...
    { 
        public static function open()
        {
            ...
            while($row1 = mysqli_fetch_array($result_table))
            {
                $resualt_text = $resualt_text. "<tr><td>".$row1[0]."</td>
                    ...
                    <td>".$row1[4]."</td>
                    <td>".$row1[5]."</td>
                    <td><a href='rewrite.php?rew_id=".$row1[0]."'>Редактировать</a></td>
                    <td><a href='?del_id=".$row1[0]."'>Удалить</a></td>
                </tr>";
            }
            return $resualt_text;
        }
        ...
        public static function del(int $del_id)
        {
            ...
            $databaseName = "orders";
            $connect = mysqli_connect($hostname, $username, $password, $databaseName);
            $query = "SELECT invoiceScan FROM transactions WHERE invoiceId = '$del_id'";
            $result_table = mysqli_query($connect, $query);
            $row1 = mysqli_fetch_array($result_table);
            $path = $_SERVER['DOCUMENT_ROOT'] . '/LR7/image/'.$row1[0];
            unlink($path);
            $query ="DELETE FROM transactions WHERE invoiceId = '$del_id'";
            ...
            <table class="table table-striped table-dark">
                <thead>
                    <tr>
                        ...
                        <th style="width:80px">Редактирование</th>
